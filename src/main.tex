...
\title{%
  ...
  Longitudinal Study of Software Environments Produced by Dockerfiles from Research Artifacts: Initial Design% and Results
}
...
Several works already shed light on the possible variability of results given a badly captured software environment \cite{mytkowicz_producing_nodate} \todo{more}.
...
But this come with the downside of requiring more storage space than just storing the recipe (binary vs. text file), and thus has some \textbf{severe scalability and sustainability issues in terms of longevity}~\cite{guilloteau2024longevity}.
...
\begin{figure*}
  \centering
  %\fbox{
  \includegraphics[width=1\textwidth,trim={1.40cm 1.65cm 1.65cm 1.65cm},clip]{./figs/workflow.pdf}
  %}
  \caption{
    Workflow of \ecg.
...
The data collected is available on Zenodo~\cite{dataset_europar24}. \todo{update zenodo with the april measures}
...
\todo{update with april measures}
